# OTUS Rust: Веб-сервис для запуска и контроля расчетов

## Описание
Проект состоит из двух Rust-сервисов и Redis:
- `calc_runner` — HTTP API на Axum для запуска расчетов (базовый и массовый), хранит состояние в Redis. Эндпоинты: `/api/calc/base_calc`, `/api/calc/mass_calc`, `/api/calc/{id}`, `/api/calc/result/{id}`, `/api/stats/user/{id}`, `/api/stats/active_calcs`, `/health`.
...
- `redis` — хранилище очередей и состояний расчётов.
